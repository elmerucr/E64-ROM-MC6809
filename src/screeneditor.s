...
.1	lda	CIA_AC				; do we have a character?
	...
	ldx	#BLIT_CMD_DEACTIVATE_CURSOR
	stx	BLIT_CR
	cmp	#ASCII_LF			; is it an LF?
	bne	.3
...
.2	stx	BLIT_CR
	...
	beq	.2
	bra	.6
.3	cmp	#ASCII_CURSOR_RIGHT
	bne	.4
	lda	#BLIT_CMD_INCREASE_CURSOR_POS
	sta	BLIT_CR
	bra	.6
.4	cmp	#ASCII_CURSOR_LEFT
	bne	.5
	lda	#BLIT_CMD_DECREASE_CURSOR_POS
	sta	BLIT_CR
	bra	.6
.5	lda	#BLIT_CMD_PUT_SYMBOL_AT_CURSOR	; normal character
	sta	BLIT_CR
	lda	#BLIT_CMD_INCREASE_CURSOR_POS
	sta	BLIT_CR
